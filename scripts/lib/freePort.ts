...
export async function ensurePortFree(
  ...
  if (holders.length === 0) {
    ...
    // a permissions issue on lsof or Windows platform). Fall through to the manual message.
    log(`Port ${port} (${name}) is in use, but the holding process could not be identified or killed.`)
    if (process.platform === 'win32') {
      log(`Run \`netstat -ano | findstr :${port}\` to inspect it manually and kill the PID.`)
    } else {
