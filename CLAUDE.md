...
- **Two-layer color model.** Surfaces, borders, and default text are Deep Ocean slate neutrals (one hue family). Chromatic color is used as **interaction** (`--brand-*`: primary buttons, focus, links, active nav), as **identity** (`--accent-1..10` categorical tints, assigned via `@ui/railAccent`), as **state** (`--danger*`, `--warning*`, `--success*`, `--info-text`), or as **canvas affordance** (`--canvas-selection-ring`, `--canvas-hover-ring`). Never decorative.
- **Card surface pattern.** Flat tile cards are borderless: `--bg-surface-2` on a darker `--bg-surface` parent with a 1px grid gap, `--card-radius` (16px). Hover lifts the surface tone — never recolor a flat card's border. Dashboard hero tiles use the glass variant (`--glass-*` tokens). Canonical implementation: `src/ui/components/Widget/Widget.module.css`.
- **Border radius scale.** `--radius-sm` (3px) for tight chips. `--radius` (6px) for default editor controls and buttons. `--radius-lg` (8px) for compact buttons and tree rows. `--panel-radius` (12px) for floating overlay panels. `--card-radius` (16px) for tile cards. `--input-radius` (1em) for pill-shaped inputs. Don't introduce ad-hoc radius values.
- **Motion and weight tokens.** Transition durations/easing come from `--duration-fast/--duration/--duration-slow` + `--ease/--ease-out`; font weights come from `--weight-regular/medium/semibold/bold`. No raw `150ms ease`, no raw `font-weight: 620`.
