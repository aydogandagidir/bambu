...
import { handleImportUrlArchiveRoute } from './importUrlArchive'
import { handleImportUrlRoute } from './importUrl'
...
export async function handleCmsRequest(
  ...
  const response =
    ...
    ?? (await handleImportUrlArchiveRoute(req, db, options))
    ?? (await handleImportUrlRoute(req, db, options))
